...
<flux:modal wire:model="showShareDialog" class="max-w-md">
    ...
    @if($groceryList->is_shared)
        <div class="space-y-4" x-data="{ copied: false }">
            <div>
                <flux:text class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Shareable Link</flux:text>
                <div class="flex items-center gap-2">
                    <input
                        type="text"
                        readonly
                        value="{{ $groceryList->share_url }}"
                        class="flex-1 px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-text select-all"
                        id="shareLink"
                        x-ref="shareLink"
