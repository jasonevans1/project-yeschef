<div>
    ...
    <div class="mb-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
                <flux:text class="text-blue-900 dark:text-blue-100 font-medium">
    ...
    @else
        ...
        <div class="space-y-6">
            @foreach($itemsByCategory as $categoryValue => $items)
                <div wire:key="shared-category-{{ $categoryValue }}" class="bg-white dark:bg-zinc-900 rounded-lg shadow overflow-hidden">
                    {{-- Category Header --}}
                    <div class="bg-gray-50 dark:bg-zinc-800 px-6 py-3 border-b border-gray-200 dark:border-zinc-700">
                        <div class="flex items-center justify-between">
                            <flux:heading size="lg">
                                {{ \App\Enums\IngredientCategory::from($categoryValue)->label() }}
                            </flux:heading>
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-zinc-700">
                        @foreach($items as $item)
                            <div wire:key="shared-item-{{ $item->id }}" class="px-6 py-4 flex items-center gap-4 hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors">
                                ...
                                <div class="flex-shrink-0">
                                    ...
                                    @else
                                        <div class="w-5 h-5 rounded border-2 border-gray-300 dark:border-zinc-600 bg-white dark:bg-zinc-800"></div>
                                    @endif
                                </div>
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-baseline gap-2">
                                        <span class="font-medium text-gray-900 dark:text-white {{ $item->purchased ? 'line-through text-gray-500 dark:text-zinc-400' : '' }}">
                                            {{ $item->name }}
                                        </span>

                                        @if($item->quantity || $item->unit)
                                            <span class="text-sm text-gray-600 dark:text-zinc-400 {{ $item->purchased ? 'line-through' : '' }}">
                                                @if($item->quantity)
                                    ...
                                    @if($item->notes)
                                        <p class="text-sm text-gray-600 dark:text-zinc-400 italic mt-1 {{ $item->purchased ? 'line-through' : '' }}">
                                            {{ $item->notes }}
                                        </p>
                                    @endif
    ...
    <div class="mt-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
        <div class="flex items-center gap-3">
            ...
            <flux:text class="text-yellow-800 dark:text-yellow-200 text-sm">
