...
<a href="{{ route('recipes.show', $recipe) }}" class="block group hover:shadow-lg transition-shadow duration-200 rounded-lg overflow-hidden bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700">
    ...
    <div class="p-4">
        {{-- Recipe Name --}}
        <h3 class="font-semibold text-lg mb-2 group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400 transition-colors line-clamp-2">
            {{ $recipe->name }}
        </h3>
        ...
        @if ($recipe->dietary_tags && count($recipe->dietary_tags) > 0)
            <div class="flex flex-wrap gap-1">
                @foreach (array_slice($recipe->dietary_tags, 0, 3) as $tag)
                    <span class="text-xs px-2 py-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded">
                        {{ ucfirst($tag) }}
                    </span>
                @endforeach
